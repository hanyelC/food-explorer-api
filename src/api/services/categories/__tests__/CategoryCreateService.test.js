...
import { CategoryCreateService } from '../CategoryCreateService.js'
import { CategoryRepositoryInMemory } from '../../../repositories/categories/CategoryRepositoryInMemory.js'
...
describe('CategoryCreateService', () => {
  ...
  it('Category should not be created without name', async () => {
    ...
    await expect(categoryCreateService.execute(category)).rejects.toEqual(
      new AppError('Nome da categoria é obrigatório.')
    )
  })
  ...
  it('Category should not be created without description', async () => {
    ...
    await expect(categoryCreateService.execute(category)).rejects.toEqual(
      new AppError('Descrição da categoria é obrigatória.')
    )
  })
  ...
  it('Category should not be created with invalid name => {}', async () => {
    ...
    await expect(categoryCreateService.execute(category)).rejects.toEqual(
      new AppError('Nome da categoria deve ser um texto.')
    )
  })
  ...
  it('Category should not be created with invalid name => 51 character name', async () => {
    ...
    await expect(categoryCreateService.execute(category)).rejects.toEqual(
      new AppError('Nome da categoria não deve ter mais que 50 caracteres.')
    )
  })
  ...
  it('Category should not be created with name that already exists', async () => {
    ...
    await expect(categoryCreateService.execute(newCategory)).rejects.toEqual(
