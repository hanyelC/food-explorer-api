...
describe('FavoriteCreateService', () => {
  ...
  beforeEach(() => {
    ...
    favoriteCreateService = new FavoriteCreateService(
      favoritesRepositoryInMemory
    )
  })

  it('Favorite should be created with valid data', async () => {
    const createdFavorite = await favoriteCreateService.execute({
      user_id,
      product_id,
    })
  ...
  it('Favorite should not be created without user id', async () => {
    await expect(favoriteCreateService.execute({ product_id })).rejects.toEqual(
      new AppError('User id is required.')
    )
  })

  it('Favorite should not be created without product id', async () => {
    await expect(favoriteCreateService.execute({ user_id })).rejects.toEqual(
  ...
  it('Favorite that already exists should not be created', async () => {
    ...
    await expect(
      favoriteCreateService.execute({ user_id, product_id })
    ).rejects.toEqual(new AppError('Favorito já existe.'))
  })
})
