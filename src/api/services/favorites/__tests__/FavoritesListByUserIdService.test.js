...
describe('FavoritesListByUserIdService', () => {
  ...
  test.each([{ value: 'string' }, { value: [] }, { value: {} }])(
    'Should throw with invalid user_id => $value',
    async ({ value }) => {
      await expect(favoritesListByUserIdService.execute(value)).rejects.toEqual(
        new AppError('User id should be a number.')
      )
    }
  )

  test.each([{ value: null }, { value: undefined }])(
    'Should throw with invalid user_id => undefined',
    async ({ value }) => {
      await expect(favoritesListByUserIdService.execute(value)).rejects.toEqual(
