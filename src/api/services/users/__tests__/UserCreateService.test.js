...
describe('UserCreateService', () => {
  ...
  it('user should not be created without name', async () => {
    ...
    await expect(userCreateService.execute(user)).rejects.toEqual(
      new AppError('Nome é obrigatório.')
    )
  })
  ...
  it('user should not be created without email', async () => {
    ...
    await expect(userCreateService.execute(user)).rejects.toEqual(
      new AppError('Email é obrigatório.')
    )
  })
  ...
  it('user should not be created without password', async () => {
    ...
    await expect(userCreateService.execute(user)).rejects.toEqual(
      new AppError('Senha é obrigatória.')
    )
  })
  ...
  it('user should not be created without password confirmation', async () => {
    ...
    await expect(userCreateService.execute(user)).rejects.toEqual(
      new AppError('Confirmação da senha é obrigatória.')
    )
  })
  ...
  it("user should not be created if password && confirmation didn't match", async () => {
    ...
    await expect(userCreateService.execute(user)).rejects.toEqual(
      new AppError('Senhas não conferem.')
    )
  })
  ...
  it('user should not be created with invalid password', async () => {
    ...
    await expect(userCreateService.execute(user)).rejects.toEqual(
      new AppError('A senha precisa ter no mínimo 6 caracteres.')
    )
  })
  ...
  it('user should not be created with invalid password => 1', async () => {
    ...
    await expect(userCreateService.execute(user)).rejects.toEqual(
      new AppError('Senha inválida, a senha deve ser uma string.')
    )
  })
